...
def generar_veredicto(res_actual, res_parcial, res_total, escenarios, totales_matriz, tasa):
    ...
    if gan > 0:
        return {"veredicto": "IMPLEMENTAR POR FASES",
                "alternativa": f"{ganadora_nombre} — por fases",
                "color": "amarillo",
                "justif": f"{ganadora_nombre} tiene ganancia positiva (${gan/1e6:,.0f}M) pero las condiciones no son lo suficientemente favorables para implementar de golpe. Avanzar gradualmente."}
    return {"veredicto": "DESCARTAR automatización",
            "alternativa": "Mantener sistema actual + mejoras",
            "color": "rojo",
            "justif": "Las condiciones actuales no justifican la inversión en automatización."}
...
with tab_guia:
    ...
    **explorar la decisión de automatización** modificando variables y viendo en tiempo real
    ...
    veredictos_info = [
        ("✅ IMPLEMENTAR", "verde", "Ganancia positiva, buen margen, ROI superior a la tasa y resiste el escenario pesimista. Automatizar completamente."),
        ("⚠️ IMPLEMENTAR POR FASES", "amarillo", "Ganancia positiva con ROI atractivo, pero no resiste el pesimista o las condiciones no son ideales. Avanzar por etapas."),
        ("🔴 DESCARTAR", "rojo", "Ninguna alternativa genera ganancia positiva. Mantener sistema actual con mejoras."),
    ]
