...
def calcular_alternativa(params, tipo):
    ...
    CVu = params["cv_materiales"] + cv_energia + cv_reproceso
    CV = CVu * Q
    CT = CF + CV
    ...
    #    (arriendos, servicios, mantenimiento suben con la inflación)
    #    Factor = promedio de la inflación acumulada en el periodo
    factor_inflacion = 1 + inflacion * (horizonte - 1) / 3
    CF_promedio = CF * factor_inflacion
    CT_promedio = CF_promedio + CV  # CV usa precios actuales (compra mes a mes)
    utilidad_promedio = IT - CT_promedio  # utilidad mensual promedio real

    # 2. GANANCIA BRUTA del proyecto = utilidad promedio en todo el horizonte - inversión
    ganancia_bruta = (utilidad_promedio * 12 * horizonte) - capex_ajustado

    # 3. COSTO DE OPORTUNIDAD: lo que ganarías en un CDT con esa plata
    #    (Concepto: tasa de interés como costo de oportunidad del capital)
    ganancia_cdt = capex_ajustado * tasa * horizonte

    # 4. GANANCIA NETA = ganancia del proyecto - lo que pierdes de no invertir en CDT
    ganancia_proyecto = ganancia_bruta - ganancia_cdt
