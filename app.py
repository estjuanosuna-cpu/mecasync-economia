...
<style>
    ...
    .concepto-card p, .concepto-card span, .concepto-card li {
...
def calcular_alternativa(params, tipo):
    ...
    # Inflación alta (>4%) también encarece materiales y energía a lo largo del horizonte
    if inflacion > 0.04:
        factor_inf_cv = 1 + (inflacion - 0.04) * (horizonte - 1) / 4
        CT_promedio = CF_promedio + CV * factor_inf_cv
...
def generar_veredicto(res_actual, res_parcial, res_total, escenarios, totales_matriz, tasa):
    """Genera recomendación: VERDE=auto total, AMARILLO=auto parcial, ROJO=descartar."""

    # Indicadores de cada alternativa
    gan_total = res_total["ganancia_proyecto"]
    gan_parcial = res_parcial["ganancia_proyecto"]
    roi_total = res_total["ROI"]
    roi_parcial = res_parcial["ROI"]
    margen_total = res_total["margen"]
    margen_parcial = res_parcial["margen"]
    pb_total = res_total["payback"]
    pb_parcial = res_parcial["payback"]

    # Escenario pesimista
    gan_total_pesim = escenarios["PESIMISTA"]["total"]["ganancia_proyecto"]
    gan_parcial_pesim = escenarios["PESIMISTA"]["parcial"]["ganancia_proyecto"]

    # ═══ VERDE: Automatización TOTAL ═══
    # Condiciones tan favorables que la inversión grande se justifica
    if (gan_total > 0 and gan_total_pesim > 0
        and margen_total > 0.02 and roi_total > tasa):
        return {"veredicto": "IMPLEMENTAR",
                "alternativa": "Automatización total",
                "color": "verde",
                "justif": f"Automatización total genera ganancia de ${gan_total/1e6:,.0f}M, ROI del {roi_total:.0%} (supera tasa del {tasa:.0%}), margen del {margen_total:.1%}, y resiste el escenario pesimista (${gan_total_pesim/1e6:,.0f}M). Recuperación en {pb_total:.1f} años."}

    # ═══ AMARILLO: Automatización PARCIAL ═══
    # Auto total no es viable, pero la parcial sí genera valor
    if gan_parcial > 0 and roi_parcial > tasa:
        return {"veredicto": "IMPLEMENTAR POR FASES",
                "alternativa": "Automatización parcial — por fases",
                "color": "amarillo",
                "justif": f"Automatización parcial es rentable (ganancia ${gan_parcial/1e6:,.0f}M, ROI {roi_parcial:.0%}), pero la automatización total no es viable en estas condiciones. Implementar gradualmente."}

    # ═══ ROJO: DESCARTAR ═══
    # Ni total ni parcial generan valor suficiente
    return {"veredicto": "DESCARTAR automatización",
            "alternativa": "Mantener sistema actual + mejoras",
            "color": "rojo",
            "justif": "Las condiciones actuales no justifican la inversión en automatización. Mantener el sistema actual con mejoras de mantenimiento."}
    ...
    **explorar la decisión de automatización** modificando variables y viendo en tiempo real
    ...
    with st.expander("**Demo 1: Crisis cambiaria — TRM se dispara** 💱", expanded=False):
        ...
        **Qué observar:**
        ...
        - La **Ganancia de la auto. total** se vuelve aún más negativo
    ...
    with st.expander("**Demo 3: Política monetaria contractiva — sube la tasa** 🏦", expanded=False):
        ...
        **Qué observar:**
        - La **Ganancia de la auto. parcial** cae significativamente
        - La **Ganancia del sistema actual** también baja (los flujos futuros valen menos hoy)
        - Posible cambio de veredicto a "DESCARTAR"
    ...
    st.markdown("""
    El simulador emite 1 de **3 posibles veredictos** según las condiciones:
    """)

    veredictos_info = [
        ("✅ IMPLEMENTAR", "verde", "Condiciones favorables para automatización TOTAL. Ganancia positiva, buen margen, ROI superior a la tasa y resiste el pesimista."),
        ("⚠️ IMPLEMENTAR POR FASES", "amarillo", "Automatización PARCIAL viable. La total no es viable pero la parcial genera valor. Avanzar por etapas."),
        ("🔴 DESCARTAR", "rojo", "Ni la automatización total ni la parcial generan ganancia positiva. Mantener sistema actual."),
    ]
...
with tab1:
    ...
    # === DIAGNÓSTICO: por qué y qué falta ===
    with st.expander("📋 **¿Por qué este veredicto? ¿Qué me falta para subir de nivel?**", expanded=True):
        gan_p = res_parcial["ganancia_proyecto"]
        gan_t = res_total["ganancia_proyecto"]
        roi_p = res_parcial["ROI"]
        roi_t = res_total["ROI"]
        mar_t = res_total["margen"]
        tasa_v = params["tasa_interes"]
        gan_t_pesim = escenarios["PESIMISTA"]["total"]["ganancia_proyecto"]
        util_p = res_parcial["utilidad_mensual"]
        util_t = res_total["utilidad_mensual"]
        precio = params["precio_venta"]
        cvu_p = res_parcial["CVu"]
        cvu_t = res_total["CVu"]

        if veredicto["color"] == "rojo":
            st.error("**DESCARTAR** — Ni la automatización parcial ni la total generan ganancia.")
            st.markdown("**¿Por qué se descarta?**")
            razones = []
            if precio < cvu_p:
                razones.append(f"- El **precio (${precio/1e6:,.2f}M) es menor que el costo variable (${cvu_p/1e6:,.2f}M)**. Cada unidad vendida genera pérdida.")
            elif util_p <= 0:
                razones.append(f"- La utilidad mensual de la auto. parcial es **negativa** (${util_p/1e6:,.1f}M). Los costos fijos superan el margen por unidad.")
            elif gan_p <= 0:
                razones.append(f"- La auto. parcial tiene utilidad mensual de ${util_p/1e6:,.1f}M, pero la **inflación ({params['inflacion']:.0%}), tasa ({tasa_v:.0%}) y TRM (${params['trm']:,.0f})** erosionan la ganancia total a ${gan_p/1e6:,.0f}M.")
            else:
                razones.append(f"- Ganancia parcial: ${gan_p/1e6:,.0f}M | ROI: {roi_p:.0%}")
            st.markdown("\n".join(razones))

            st.markdown("**¿Qué cambiar para llegar a automatización parcial?**")
            sugerencias = []
            if precio < cvu_p * 1.15:
                sugerencias.append(f"- Subir **precio de venta** a mínimo ${cvu_p*1.20/1e6:,.2f}M")
            sugerencias.append("- Subir **demanda** para diluir costos fijos en más unidades")
            if params["inflacion"] > 0.05:
                sugerencias.append(f"- Bajar **inflación** (actualmente {params['inflacion']:.0%}, erosiona costos)")
            if params["tasa_interes"] > 0.15:
                sugerencias.append(f"- Bajar **tasa de interés** (actualmente {tasa_v:.0%})")
            if params["trm"] > 4500:
                sugerencias.append(f"- Bajar **TRM** (actualmente ${params['trm']:,.0f})")
            if params["salario_integral"] > 3_000_000:
                sugerencias.append(f"- Bajar **salario** (actualmente ${params['salario_integral']/1e6:,.1f}M)")
            st.markdown("\n".join(sugerencias))

        elif veredicto["color"] == "amarillo":
            st.warning("**POR FASES (parcial)** — La parcial es viable pero la total todavía no.")
            st.markdown("**¿Por qué no automatización total?**")
            razones = []
            if util_t <= 0:
                razones.append(f"- Utilidad mensual de auto. total es **negativa** (${util_t/1e6:,.1f}M)")
            elif gan_t <= 0:
                razones.append(f"- Ganancia de auto. total es **negativa** (${gan_t/1e6:,.0f}M) por inflación, tasa y prima de riesgo")
            elif gan_t_pesim <= 0:
                razones.append(f"- Auto. total no resiste el **escenario pesimista** (${gan_t_pesim/1e6:,.0f}M)")
            elif mar_t <= 0.02:
                razones.append(f"- **Margen bajo** ({mar_t:.1%}, necesita > 2%)")
            elif roi_t <= tasa_v:
                razones.append(f"- **ROI ({roi_t:.0%}) no supera la tasa** ({tasa_v:.0%})")
            st.markdown("\n".join(razones) if razones else f"- Ganancia total: ${gan_t/1e6:,.0f}M | ROI: {roi_t:.0%}")

            st.markdown("**¿Qué cambiar para llegar a automatización total?**")
            sugerencias = [
                "- Subir **demanda** para aprovechar capacidad de 950 uds/mes",
                "- Subir **precio** para mejorar margen por unidad",
            ]
            if params["inflacion"] > 0.05:
                sugerencias.append(f"- Bajar **inflación** (actualmente {params['inflacion']:.0%})")
            if params["tasa_interes"] > 0.15:
                sugerencias.append(f"- Bajar **tasa** (actualmente {tasa_v:.0%})")
            if params["trm"] > 4500:
                sugerencias.append(f"- Bajar **TRM** (actualmente ${params['trm']:,.0f})")
            sugerencias.append(f"- Bajar **CAPEX total** (actualmente ${params['capex_total']/1e6:,.0f}M)")
            st.markdown("\n".join(sugerencias))

        else:
            st.success("**IMPLEMENTAR (total)** — Condiciones favorables para automatización completa.")
            st.markdown(f"""**¿Por qué?**
- Ganancia total: **${gan_t/1e6:,.0f}M** positiva
- ROI: **{roi_t:.0%}** supera la tasa del {tasa_v:.0%}
- Margen: **{mar_t:.1%}** superior al 2%
- Resiste pesimista: **${gan_t_pesim/1e6:,.0f}M**
- Las condiciones justifican la inversión completa.""")
